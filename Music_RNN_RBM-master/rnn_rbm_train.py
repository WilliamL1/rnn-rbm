...
def main(num_epochs):
    ...
    updt = opt_func.apply_gradients(gvs)

    # Load the songs
    songs = midi_manipulation.get_songs('Pop_Music_Midi')

    # We use this saver object to restore the weights of
    # the model and save the weights every few epochs
    saver = tf.train.Saver(tvars)
    with tf.Session() as sess:
        init = tf.initialize_all_variables()
        sess.run(init)
        # Here we load the initial weights of the model
        # that we created with weight_initializations.py
